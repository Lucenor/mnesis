...
Mnesis uses [litellm](https://docs.litellm.ai/) under the hood, which means it works with any provider litellm supports — Anthropic, OpenAI, Google Gemini, OpenRouter, Azure, and more. You never interact with litellm directly; you just pass a model string and set the appropriate API key environment variable.
...
The provider prefix is optional for well-known model names (e.g. `"gpt-4o"` works without `"openai/"`), but using the full form is recommended for clarity.
...
# OpenRouter (access many providers through one key)
os.environ["OPENROUTER_API_KEY"] = "sk-or-..."
session = await MnesisSession.create(model="openrouter/meta-llama/llama-3.1-70b-instruct")
```

### Using a different model for compaction

Compaction (context summarisation) defaults to the session model. You can use a cheaper/faster model for it:

```python
from mnesis import MnesisSession
from mnesis.models.config import MnesisConfig, CompactionConfig

config = MnesisConfig(
    compaction=CompactionConfig(
        compaction_model="openai/gpt-4o-mini",  # cheap model for summaries
    )
)
session = await MnesisSession.create(model="openai/gpt-4o", config=config)
```

### Passing extra litellm kwargs

If you need to pass provider-specific parameters (e.g. `api_base` for a self-hosted model, or `extra_headers` for OpenRouter), set them as litellm environment variables or configure litellm globally before creating a session:
...
See the [litellm documentation](https://docs.litellm.ai/docs/providers) for the full list of supported providers and their configuration options.
